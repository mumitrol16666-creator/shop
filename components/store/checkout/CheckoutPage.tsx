...
export function CheckoutPage({ settings }: { settings: StoreSettings }) {
  ...
  useEffect(() => {
    if (fulfilment === "pickup" && !settings.pickupEnabled) setFulfilment("aktobe_delivery");
    if (fulfilment === "aktobe_delivery" && !settings.deliveryEnabled) setFulfilment("pickup");
    if (payment === "kaspi_pay" && !settings.kaspiEnabled) setPayment("cash_transfer");
    if (payment === "cash_transfer" && !settings.cashTransferEnabled) setPayment("kaspi_pay");
  }, [fulfilment, payment, settings]);
  ...
  return (
    <div className="store-page store-checkout-page">
      <header><p className="store-eyebrow">ОФОРМЛЕНИЕ</p><h1>Как получить заказ?</h1><p>Товар будет зарезервирован на {settings.reservationMinutes} минут. Менеджер подтвердит детали.</p></header>
      <form className="store-checkout-layout" onSubmit={submit}>
        <div className="store-checkout-form">
          ...
          <section className="checkout-section"><span className="checkout-step">2</span><h2>Получение</h2><div className="checkout-options">{settings.pickupEnabled && <label className={fulfilment === "pickup" ? "is-selected" : ""}><input type="radio" name="fulfilment" checked={fulfilment === "pickup"} onChange={() => setFulfilment("pickup")} /><strong>Самовывоз</strong><small>Из магазина Maestro в {settings.city}</small></label>}{settings.deliveryEnabled && <label className={fulfilment === "aktobe_delivery" ? "is-selected" : ""}><input type="radio" name="fulfilment" checked={fulfilment === "aktobe_delivery"} onChange={() => setFulfilment("aktobe_delivery")} /><strong>Доставка по {settings.city}</strong><small>Стоимость и время подтвердит менеджер</small></label>}</div>{fulfilment === "aktobe_delivery" && <label>Адрес доставки<input required value={address} onChange={(event) => setAddress(event.target.value)} placeholder="Улица, дом, квартира" /></label>}</section>
          <section className="checkout-section"><span className="checkout-step">3</span><h2>Оплата</h2><div className="checkout-options">{settings.kaspiEnabled && <label className={payment === "kaspi_pay" ? "is-selected" : ""}><input type="radio" name="payment" checked={payment === "kaspi_pay"} onChange={() => setPayment("kaspi_pay")} /><strong>Kaspi Pay / QR</strong><small>Оплата после создания заказа</small></label>}{settings.cashTransferEnabled && <label className={payment === "cash_transfer" ? "is-selected" : ""}><input type="radio" name="payment" checked={payment === "cash_transfer"} onChange={() => setPayment("cash_transfer")} /><strong>Наличные или перевод</strong><small>Согласуете с менеджером</small></label>}</div><label>Комментарий <small>необязательно</small><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Цвет, вопрос или пожелание" rows={3} /></label></section>
        </div>
